...
__global__ void rasterize_backward_kernel(
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range.x) {
            ...
            id_batch[tr] = g_id;
            const auto& gaussian = gaussians[g_id];
            const float2 xy = {gaussian.means_2d.x, gaussian.means_2d.y};
            const float opac = opacities[g_id];
            xy_opacity_batch[tr] = {xy.x, xy.y, opac};
            conic_batch[tr] = gaussian.conic;
            rgbs_batch[tr] = gaussian.color;
        }
...
__device__ float calculate_compensation(const float3& conic)
{
    const float3 conic_no_add = {conic.x - 0.3f, conic.y, conic.z - 0.3f};
    const auto det_a = conic.x * conic.z - conic.y * conic.y;
    const auto det_b = conic_no_add.x * conic_no_add.z - conic_no_add.y * conic_no_add.y;
    const auto result = det_a / det_b;
...
)
{
    ...
    const auto current_conic = gaussians[idx].conic;
    const auto current_v_compensation = 0.0f;
    const auto current_compensation = calculate_compensation(current_conic);
    // get v_cov2d
    cov2d_to_conic_vjp(current_conic, v_conic[idx], v_cov2d[idx]);
    cov2d_to_compensation_vjp(current_compensation, current_conic, current_v_compensation, v_cov2d[idx]);
...
__device__ void project_cov3d_ewa_vjp(
    ...
    float rz3 = rz2 * rz;
    glm::vec3 v_t = glm::vec3(-fx * rz2 * v_J[2][0], -fy * rz2 * v_J[2][1],
                              -fx * rz2 * v_J[0][0] + 2.f * fx * t.x * rz3 * v_J[2][0] - fy * rz2 * v_J[1][1] +
                                  2.f * fy * t.y * rz3 * v_J[2][1]);
    // printf("v_t %.2f %.2f %.2f\n", v_t[0], v_t[1], v_t[2]);
    // printf("W %.2f %.2f %.2f\n", W[0][0], W[0][1], W[0][2]);
...
__device__ void scale_rot_to_cov3d_vjp(const float3 scale, const float glob_scale, const float4 quat,
                                       const float* __restrict__ v_cov3d, float3& __restrict__ v_scale,
                                       float4& __restrict__ v_quat)
{
    ...
    glm::mat3 v_V = glm::mat3(v_cov3d[0], 0.5 * v_cov3d[1], 0.5 * v_cov3d[2], 0.5 * v_cov3d[1], v_cov3d[3],
                              0.5 * v_cov3d[4], 0.5 * v_cov3d[2], 0.5 * v_cov3d[4], v_cov3d[5]);
